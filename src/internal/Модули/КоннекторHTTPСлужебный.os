...
Функция ВызватьHTTPМетод(Сессия, Метод, URL, ДополнительныеПараметры) Экспорт
	...
	НастройкиПодключения = ПолучитьНастройкиПодключения(Метод, ДополнительныеПараметры);
...
Функция ПолучитьНастройкиПодключения(Метод, ДополнительныеПараметры) 
	...
	Прокси = ПолучитьЗначениеПоКлючу(ДополнительныеПараметры, "Прокси", Неопределено);
...
Функция ОтправитьHTTPЗапрос(ПодготовленныйЗапрос, Настройки)
	...
	СтруктураURL = КоннекторHTTP.РазобратьURL(ПодготовленныйЗапрос.URL);
	Соединение = ПолучитьСоединение(СтруктураURL, ПодготовленныйЗапрос.Аутентификация, Настройки);

...
Ответ = КоннекторHTTP.Post("https://httpbin.org/post", Неопределено, Неопределено, Новый Структура("Файлы", Файлы));
```
...
Ответ = КоннекторHTTP.Post("https://httpbin.org/post", Неопределено, Неопределено, Новый Структура("Файлы,Данные", Файлы, Данные));
```

...
// BSLLS:UnusedParameters-on
	...
	МассивТестов.Добавить("ТестДолжен_ПроверитьЧтоРаботаетРазборURLСИменемПользователяИПаролем");
	МассивТестов.Добавить("ТестДолжен_ПроверитьЧтоРаботаетРазборURLСИменемПользователяБезПароля");
	МассивТестов.Добавить("ТестДолжен_ПроверитьЧтоРаботаетРазборURLСЗакодированнымURLВПараметре");
...
Процедура ТестДолжен_ПроверитьЧтоРаботаетРазборURLСоЗнакомРавноВЗначенииПараметраЗапроса() Экспорт
	...
	Ожидаем.Что(СтруктураURL.Сервер).Равно("connectorhttp.ru");
...
Процедура ТестДолжен_ПроверитьЧтоРаботаетРазборURLСИменемПользователяИПаролем() Экспорт
	...
	Ожидаем.Что(СтруктураURL.Порт).Равно(0);
	Ожидаем.Что(СтруктураURL.Фрагмент).Равно("");
	Ожидаем.Что(СтруктураURL.Аутентификация.Пользователь).Равно("user");
	Ожидаем.Что(СтруктураURL.Аутентификация.Пароль).Равно("pass");
	Ожидаем.Что(СтруктураURL.ПараметрыЗапроса.Количество()).Равно(0);

КонецПроцедуры

Процедура ТестДолжен_ПроверитьЧтоРаботаетРазборURLСИменемПользователяБезПароля() Экспорт
...
Процедура ТестДолжен_ПроверитьЧтоРаботаетРазборURLСЗакодированнымURLВПараметре() Экспорт
	
	СтруктураURL = КоннекторHTTP.РазобратьURL(
		"https://www.example.ru?url=http%3A%2F%2Fwww.kuku.ru%2F%3Fs%3D1%26b%3D2&OTHER=1");

	Ожидаем.Что(СтруктураURL.Схема).Равно("https");
	Ожидаем.Что(СтруктураURL.Сервер).Равно("www.example.ru");
	Ожидаем.Что(СтруктураURL.Путь).Равно("/");
	Ожидаем.Что(СтруктураURL.Порт).Равно(0);
	Ожидаем.Что(СтруктураURL.Фрагмент).Равно("");
	Ожидаем.Что(СтруктураURL.Аутентификация.Пользователь).Равно("");
	Ожидаем.Что(СтруктураURL.Аутентификация.Пароль).Равно("");
	Ожидаем.Что(СтруктураURL.ПараметрыЗапроса.Количество()).Равно(2);
	Ожидаем.Что(СтруктураURL.ПараметрыЗапроса["url"]).Равно("http://www.kuku.ru/?s=1&b=2");
	Ожидаем.Что(СтруктураURL.ПараметрыЗапроса["OTHER"]).Равно("1");

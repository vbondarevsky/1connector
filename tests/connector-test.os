...
Процедура ТестДолжен_ПроверитьЧтоРаботаетРедиректСОтносительнымАдресом() Экспорт
	...
	Ответ = КоннекторHTTP.Get("http://localhost:10080/relative-redirect/6");
	Результат = Ответ.Json();
	
	Ожидаем.Что(Ответ.КодСостояния).Равно(200);
	Ожидаем.Что(Ответ.URL).Равно("http://localhost:10080/get");
	
КонецПроцедуры

Процедура ТестДолжен_ПроверитьЧтоРаботаетРедиректСАбсолютнымАдресом() Экспорт
	
	Ответ = КоннекторHTTP.Get("http://localhost:10080/absolute-redirect/6");
	Результат = Ответ.Json();
...
Процедура ТестДолжен_ПроверитьЧтоРаботаетПрерываниеЗацикленногоРедиректа() Экспорт
	...
	ТекстОшибки = "";
	Попытка
		Ответ = КоннекторHTTP.Get("https://localhost:10080/redirect/31");
	Исключение
		ТекстОшибки = ОписаниеОшибки();
	КонецПопытки;
...
Процедура ТестДолжен_ПроверитьЧтоРаботаетРедиректСУказаниемURL() Экспорт
	...
	Ответ = КоннекторHTTP.Get("http://localhost:10080/redirect-to", ПараметрыЗапроса);
	Результат = Ответ.Json();
